...
raw_input = fread(paste(data_folder, data_file, sep="/"))
...
weighted_input$Order = as.integer(str_split_i(weighted_input$originalRUN, "Seq", 2))

weighted_input = swap_condition_labels(weighted_input, true_positives, true_negatives)
...
base_summarized = dataProcess(base_msstats_input, 
                               ...
                               MBimpute=TRUE,
                               numberOfCores = 12)
save(base_summarized, 
     file=paste(data_folder, "MSstats", "MSstats_summarized.rda", sep="/"))

# Swap condition labels for TP/TN analysis
weighted_input = base_summarized$ProteinLevelData
...
weighted_input$Label = ifelse(
  weighted_input$Protein %in% true_positives, 
  "Positive", "Negative")

weighted_input$Order = NULL
base_summarized$ProteinLevelData = weighted_input

base_summarized$ProteinLevelData$Variance = NA
base_summarized$ProteinLevelData$SUBJECT = as.numeric(as.factor(
  paste0(base_summarized$ProteinLevelData$originalRUN,
         base_summarized$ProteinLevelData$GROUP)))
...
msstats_model = groupComparison(comparison, base_summarized, numberOfCores=12)
...
## Convert into MSqRob2 format + preprocessing
...
msqrob2_model$Protein = row.names(msqrob2_model)
...
limma_model$Label = ifelse(
  limma_model$Protein %in% sample_proteins_sig, "Positive", "Negative")
...
fwrite(mapdia_input, 
       file=paste(data_folder, "mapDIA", "k562_input_file.txt", sep="/"), 
       sep="\t")

# Load model results after processing outside R
mapdia_model = fread(file=paste(data_folder, "mapDIA", "analysis_output.txt", sep="/"),
                     sep="\t")
