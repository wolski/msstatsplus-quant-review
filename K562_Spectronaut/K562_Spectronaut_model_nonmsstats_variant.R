...
if (length(args) != 1 || !args[[1]] %in% c("log2", "vsn")) {
  stop("Usage: Rscript K562_Spectronaut_model_nonmsstats_variant.R <log2|vsn>",
       call. = FALSE)
}
...
benchmark_metric = function(file, method, source, p_col, fc_col) {
  dt = fread(file)
  p = dt[[p_col]]
  fc = dt[[fc_col]]
  label = dt[["Label"]]
  ...
  data.table(
    Method = method,
    Source = source,
    K562_TPR = sum(significant & label == "Positive", na.rm = TRUE) /
      sum(finite_fc & label == "Positive", na.rm = TRUE),
    K562_PPV = if (n_discoveries > 0) {
...
write_comparison_table = function(output_dir, variant_label) {
  ...
  rounded[, c("K562_TPR", "K562_PPV") := lapply(.SD, round, 3),
          .SDcols = c("K562_TPR", "K562_PPV")]

  fwrite(rounded, file.path(output_dir, "K562_Spectronaut_comparison_table.csv"))
  writeLines(
    capture.output(print(rounded)),
    file.path(output_dir, "K562_Spectronaut_comparison_table.txt")
  )
...
write_readme = function(output_dir, variant, variant_label) {
  readme_lines = c(
    paste0("# K562 Spectronaut non-MSstats variant: ", variant),
    "",
    paste0("Variant label: ", variant_label),
    "",
    "Input:",
    "- Reads raw, filtered, benchmark-swapped non-MSstats inputs from `nonmsstats_preprocessed/`.",
    "",
    "Methods rerun:",
    "- msqrob2",
    "- MaxLFQ + limma",
    ...
    "- `limpa_model.csv`",
    "- `deqms_model.csv`",
    "- `K562_Spectronaut_comparison_table.csv`",
    "- `K562_Spectronaut_comparison_table.txt`",
    "",
    "Dependency versions:",
...
preprocess_dir = "nonmsstats_preprocessed"
...
if (!all(file.exists(required_inputs))) {
  stop(
    "Missing preprocessed input(s). Run K562_Spectronaut_preprocess_nonmsstats.R first.",
    call. = FALSE
  )
}
