...
# Prepare data
msstats_input = msstats_input %>% merge(run_order, by.x="Run", by.y="Run")
msstats_input$PSM = paste(msstats_input$PeptideSequence,
                          msstats_input$PrecursorCharge,
                          sep="_")
msstats_input$FEATURE = paste(msstats_input$PeptideSequence,
                              msstats_input$PrecursorCharge,
                              msstats_input$FragmentIon,
                              msstats_input$ProductCharge,
                              sep="_")
...
ggplot(skew_score, aes(x = skew)) + 
  geom_histogram(fill = "#0072B2", color = "black", binwidth = 0.2) + 
...
msstats_input %>%
  ...
  labs(
    title = "Dataset 2: CSF Benchmark (Spectronaut)",
    x = "Anomaly Score",
    y = "Density",
    fill = "Anomaly"
  ) +
...
msstats_input %>% 
  filter(PeptideSequence == "AAAGGPGGAALGEAPPGR" & PrecursorCharge == 2) %>%
  ggplot() + 
  # Highlighted regions
  annotate("rect", xmin = 8.5, xmax = 9.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  annotate("rect", xmin = 11.5, xmax = 12.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  annotate("rect", xmin = 32.5, xmax = 40.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  # Anomaly line and points
  geom_line(aes(x = Order, y = AnomalyScores, group = PrecursorCharge), 
            color = "darkorchid4", linewidth = 1.5) + 
  geom_point(aes(x = Order, y = AnomalyScores), 
...
msstats_input %>% 
  ...
  annotate("rect", xmin = 32.5, xmax = 40.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  # Anomaly line and points
  geom_line(aes(x = Order, y = EGDeltaRT, group = PrecursorCharge), 
            color = color_p[[6]], linewidth = 1.5) + 
  geom_point(aes(x = Order, y = EGDeltaRT), 
             color = color_p[[6]], size = 4) + 
  # Theme and labels
  theme_bw() +
  theme(
    legend.position = "none",
...
msstats_input %>% 
  ...
  annotate("rect", xmin = 32.5, xmax = 40.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  # Anomaly line and points
  geom_line(aes(x = Order, y = `FGShapeQualityScore(MS2).mean_decrease`, group = PrecursorCharge), 
            color = color_p[[5]], linewidth = 1.5) + 
  geom_point(aes(x = Order, y = `FGShapeQualityScore(MS2).mean_decrease`), 
             color = color_p[[5]], size = 4) + 
  # Theme and labels
  theme_bw() +
  theme(
    legend.position = "none",
  ...
  labs(
    title = "MS2 Shape Quality Mean Decrease",
    x = "Run",
    y = "Mean Decrease"
  )
...
msstats_input %>% 
  ...
  annotate("rect", xmin = 32.5, xmax = 40.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  geom_line(aes(x=Order, y=log2(Intensity), group=FragmentIon, color=FragmentIon), 
            linewidth=1.25) + 
  geom_point(aes(x=Order, y=log2(Intensity), color=FragmentIon), size=3) + 
  ...
  labs(title="Intensity", x="Run", y = expression(Log[2]~Intensity)
  )
...
msstats_input %>% 
  ...
  annotate("rect", xmin = 32.5, xmax = 40.5, ymin = -Inf, ymax = Inf, 
           fill = "gray80", alpha = 0.5) +
  # Anomaly line and points
  geom_line(aes(x = Order, y = `EGDeltaRT.dispersion_increase`, group = PrecursorCharge),  
            color = color_p[[5]], linewidth = 1.5) + 
  geom_point(aes(x = Order, y = `EGDeltaRT.dispersion_increase`), 
             color = color_p[[5]], size = 4) + 
  # Theme and labels
  theme_bw() +
  theme(
  ...
  labs(
    title = "Delta RT Dispersion Increase",
    x = "MS Run (Ordered by acquisition time)",
    y = "Dispersion Increase"
  )
...
msstats_input %>% filter(ProteinName == "Q9UIW2") %>% distinct(PSM)

for (y in seq_along(new_cols)){
  
  plot = msstats_input %>% 
    filter(PeptideSequence == "EVAPITR" & PrecursorCharge == 2) %>% 
    ggplot() + 
...
msstats_input %>% 
  filter(PeptideSequence == "LSC[Carbamidomethyl (C)]AGSGFTFSSYAM[Oxidation (M)]HWVR" & PrecursorCharge == 3) %>% 
  ggplot() + 
  geom_line(aes(x=Order, y=`AnomalyScores`, group=PrecursorCharge), 
...
ggplot() +
  ...
  labs(
    title = expression("CSF benchmark - TP" ~ log[2] ~ FC),
    x = "",
...
ggplot() +
  ...
  theme_minimal(base_size = 28) +  # Make text readable
  labs(
    title = expression("CSF benchmark - FP" ~ log[2] ~ FC),
    x = "",
    y = expression(log[2] * " Fold Change")
  ) +
  theme(
